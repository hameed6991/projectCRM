...
function RebuildcompTable() {
    ...
    for (var j = 0; j < RowNewCount; j++) {
        ...
        if (aComponent[j] != "") {
            if (clr == "grid_background1")
                ...
                clr = "grid_background1";
            document.all.hdncomponentlist.value = aQty[j];

            //alert(document.all.hdncomponentlist.value)
    ...
    if (output != "") {
        ...
        document.all.hdncomponentlist.value = 'testvalue';
        document.all.AddInstallationDetail.contentWindow.location.reload(true);
        //alert("addlist")
    }
    else {
